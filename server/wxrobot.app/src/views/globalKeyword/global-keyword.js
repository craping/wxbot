...
export default {
    ...
    methods: {
        ...
        loadKeyword() {
            if (Object.keys(this.$config.globalKeyword).length) {
                this.chatRoomKeyMap = this.$config.globalKeyword;
                return;
            } 
            Toast.loading({
                forbidClick: true,
            ...
            this.$http.post("/keyword/getKeywords?format=json", {token: this.token}).then(response => {
                const data = response.data;
                console.log(data);
                if (!data.result) {
                    if (Object.keys(data.data).length > 0) {
                        this.$config.globalKeyword = this.chatRoomKeyMap = data.data.info.global;
                    }
                    setTimeout(() => {
                        Toast("加载全域关键字列表成功");
                    }, 500);
        ...
        beforeClose(action, done) {
            ...
            if (action == "confirm") {
                const param = { seq:"global", keyList:[this.key],token: this.token }
                this.$http.post("keyword/del?format=json",  param).then(response => {
                    const data = response.data;
                    done();
                    this.$delete(this.chatRoomKeyMap, this.key);
                    this.$config.globalKeyword = this.chatRoomKeyMap;
                    setTimeout(() => {
                        Toast(data.msg);
                    }, 500);
                })
